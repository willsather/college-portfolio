...
Bus::Bus(std::string name, Route * out, Route * in,
         int capacity, double speed):IBus() {
  name_ = name;
  outgoing_route_ = out;
  incoming_route_ = in;
...
int Bus::LoadPassenger(Passenger * new_passenger) {
    bool added_passenger = 0;
    total_passengers++;
    ...
    if (distance_remaining_ <= 0) {
      ...
      if (outgoing_route_->IsAtEnd()) {
        ...
        if (!incoming_route_->IsAtEnd()) {
          ...
          // to next stop is left but, if we didn't
          // handle any passengers here, any negative
          // will affect the distance remaining
          // (see addition below)
          ...
          if (passengers_handled != 0) {
            ...
            // If there are no passengers to be passengers_handled
            // This just outputs a skipped signal to the tereminal
            std::cout << "Stop Skipped (Stop ID:" << next_stop_->GetId() << ")"
              << std::endl;
          }
...
void Bus::Report(std::ostream& out) {
    ...
    out << "Total Passengers: " << total_passengers << std::endl;
