...
//        double latitude) : id_(id),
//        longitude_(longitude), latitude_(latitude) {
Stop::Stop(int id, double longitude, double latitude):IObservable() {
  id_ = id;
...
void Stop::Update() {
  Position pos;
  pos.x = (float)GetLongitude(); //NOLINT
  pos.y = (float)GetLatitude(); //NOLINT

  StopData dat;
  dat.id = std::to_string(GetId());
  dat.position = pos;
  dat.num_people = GetNumPassengersPresent();
